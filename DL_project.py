...
#%% Calculate the size of data after the Conv layers, so we know what is the size of the input to the FC layer.

dummy_input = torch.zeros([1, 3, 30, 30])
dummy_model = Original_Classifier()
...
model = Original_Classifier().to(device) # no need for parameters as we alredy defined them in the class
...
model = Original_Classifier().to(device)
state = torch.load('./our_checkpoints/original_network_ckpt.pth', map_location=device)
